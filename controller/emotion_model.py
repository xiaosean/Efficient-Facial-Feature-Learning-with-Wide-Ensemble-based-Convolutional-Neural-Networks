...
"""
This module re-implements cvisoin.
"""

__author__ = "xiaosean"
__email__ = "aa26365566@gmail.com"
...
class EmotionRecognition(torch.nn.Module):
    def __init__(self, device="cuda:0"):
        super(EmotionRecognition, self).__init__()
        self.device = device
        self.model = _ESR_9 = ESR(device)

    def _pre_process_input_image(self, image):
        """
        ...
        # substract the mean value of the dataset - pixel-level
        image = transforms.Normalize(mean=ESR.INPUT_IMAGE_NORMALIZATION_MEAN,
                                    std=ESR.INPUT_IMAGE_NORMALIZATION_STD)(transforms.ToTensor()(image))\
                            .unsqueeze(0)\
                            .to(self.device)

        return image

    def forward(self, image, face_coordinates):
        # preprocess - substract the mean value of the dataset - pixel-level
        image = self._pre_process_input_image(image)
       
        # Mask rcnn roi_align
        # https://pytorch.org/docs/stable/torchvision/ops.html  
        input_face = roi_align(input=image, boxes=face_coordinates, output_size=ESR.INPUT_IMAGE_SIZE)
        ...
        emotion, affect = self.model(input_face)
        #  TODO: This part can optimize by torch
